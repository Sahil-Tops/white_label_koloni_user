...
open class PopupController: UIViewController {
    ...
    fileprivate var backgroundStyle: PopupBackgroundStyle = .blackFilter(alpha: 0.1) {
        didSet {
...
private extension PopupController {
    ...
    func defaultConfigure() {
        ...
        if Global.appdel.isFadBGClear == true {
            backgroundStyle = .blackFilter(alpha: 0.1)
        } else  {
            backgroundStyle = .blackFilter(alpha: 0.9)
        }
    }
        ...
        case .blackFilter( _):
            if Global.appdel.isFadBGClear == true {
             baseScrollView.backgroundColor = UIColor.black.withAlphaComponent(0.0)
                ...
                baseScrollView.backgroundColor = UIColor.black.withAlphaComponent(0.9)
            }
...
private extension PopupController {
    ...
    func fadeIn(_ layout: PopupLayout, completion: @escaping () -> Void) {
        ...
        if Global.appdel.isFadBGClear == true {
            ...
            UIView.animate(withDuration: 0.0, delay: 0.00, options: UIView.AnimationOptions(), animations: { () -> Void in
            ...
            UIView.animate(withDuration: 0.01, delay: 0.00, options: UIView.AnimationOptions(), animations: { () -> Void in
                self.popupView.alpha = 1.0
                self.baseScrollView.alpha = 1.0
                self.popupView.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
            }) { (finished) -> Void in
    ...
    func fadeOut(_ completion: @escaping () -> Void) {
        ...
        UIView.animate(
            withDuration: 0.0, delay: 0.0, options: UIView.AnimationOptions(), animations: { () -> Void in
                Global.appdel.isFadBGClear = false
