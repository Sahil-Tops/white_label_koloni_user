...
@implementation GMUDefaultClusterRenderer {
    ...
    //This is to check more than one assets on same location.
...
/* Here Passing the arguments to create cluster icon */
- (void)renderCluster:(id<GMUCluster>)cluster animated:(BOOL)animated {
    ...
    if ([self shouldRenderAsCluster:cluster atZoom:zoom]) {
        ...
        UIImage *icon = [_clusterIconGenerator iconForSize:cluster.count]; //[UIImage imageNamed:@"location_bike"];
        ...
        GMSMarker *marker = [self markerWithPosition:cluster.position
                                         ...
                                         clusterIcon:icon
                                             iconFor:@"cluster"
                                               color:[UIColor colorWithRed:6.0f/255.0f green:210.0f/255.0f blue:210.0f/255.0f alpha:1.0f]
                                              imgUrl:@""
                                            animated:animated];
        ...
        for (id<GMUClusterItem> item in cluster.items) {
            ...
            GMSMarker *marker = [self markerWithPosition:item.position
                                                ...
                                                userData:item
                                                   count:cluster.count
                                             clusterIcon:[UIImage imageNamed:@"location_bike"] //marker pin with pin images
                                                 iconFor:@"assets"
                                                   color:item.color
                                                  imgUrl:item.strName
...
//UIImage imageNamed:[self SetImageNameForSelected:item.strName]
...
/*Here creating the cluster icon*/
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position
                             from:(CLLocationCoordinate2D)from
                         userData:(id)userData
                            count:(NSInteger)count
                      clusterIcon:(UIImage *)clusterIcon
                          iconFor:(NSString*)iconFor
                            color:(UIColor *)color
    ...
    if (clusterIcon != nil) {
        if ([iconFor  isEqual: @"assets"]){
            
            NSURL *url = [NSURL URLWithString:imgUrlStr];
            UIImageView *imageView;
            imageView = [[UIImageView alloc]initWithFrame:CGRectMake(8, 8, 30, 30)];
            imageView.contentMode = UIViewContentModeScaleAspectFit;
            [imageView sd_setImageWithURL:url placeholderImage:clusterIcon];
            
            UIView *iconView;
            iconView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, 70, 70)];
            [iconView setBackgroundColor: UIColor.clearColor];
            
            UIView * bgView;
            bgView = [[UIView alloc] initWithFrame: CGRectMake(10, 10, 45, 45)];
            bgView.layer.cornerRadius = 22.5;
            bgView.layer.masksToBounds = true;
            [bgView setBackgroundColor: color];
            
            UIView * notchView;
            notchView = [[UIView alloc] initWithFrame: CGRectMake(bgView.center.x - 17.5, bgView.center.y, 35, 35)];
            notchView.layer.cornerRadius = 40;
            [notchView setBackgroundColor: color];
            
            [iconView addSubview: notchView];
            [bgView addSubview: imageView];
            [iconView addSubview: bgView];
            
//            NSMutableDictionary *dictionary = [[NSMutableDictionary alloc] init];
//            [dictionary setValue: [NSNumber numberWithDouble:position.latitude] forKey:@"latitude"];
//            [dictionary setValue: [NSNumber numberWithDouble:position.longitude] forKey:@"longitude"];
//            [locationArray addObject: dictionary];
//            NSLog(@"%@", locationArray);
//            NSMutableArray *filteredArray = [[NSMutableArray alloc] init];
//            UILabel * countLabel;
//            countLabel = [[UILabel alloc] initWithFrame:CGRectMake(32, 5, 30, 15)];
//
//            for (NSMutableDictionary *dict in locationArray){
//                if ([[dict objectForKey:@"latitude"] isEqualToNumber:[NSNumber numberWithDouble:position.latitude]] && [[dict objectForKey:@"longitude"] isEqualToNumber:[NSNumber numberWithDouble:position.longitude]]){
//                    NSLog(@"Equal");
...
//                        countLabel.layer.borderColor = color.CGColor;
//                        countLabel.text = [NSString stringWithFormat:@"%ld", (long)filteredArray.count];
//                        [iconView addSubview: countLabel];
//                        [iconView bringSubviewToFront:countLabel];
//                    }else{
//
//                    }
//                }
...
//            imageView.contentMode = UIViewContentModeScaleAspectFit;
//            [imageView setImage: clusterIcon];
//
//            UIView *iconView;
//            iconView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, 80, 80)];
//            iconView.layer.cornerRadius = 35;
//            iconView.layer.masksToBounds = true;
//            [iconView setBackgroundColor: UIColor.clearColor];
//
//            UILabel * countLabel;
//            countLabel = [[UILabel alloc] initWithFrame:CGRectMake(43, 9, 30, 16)];
...
//Method for creating image from url
- (UIImage *)createImage:(NSString *)imageUrl{
...
- (UIImage *)returnImage:(UIImage *)image{
    
    //    UIImage *img;
    //    NSData *data = [NSData dataWithContentsOfFile:imageUrl];//[[NSData dataWithContentsOfFile:imageUrl]];
    //    img = [UIImage imageWithData:data];
    return image;
}

// NEW Methods for marker view
- (NSString *)SetImageNameForSelected:(NSString *)strName {
    
    //    if([strName isEqualToString:@"both"]){
    //       return @"location_both";
    //    }else  if([strName isEqualToString:@"bike"]){
    //    return strName;
    return @"location_bike";
    //    }
    //    else{
    //       return @"location_kube";
    //    }
}

- (NSString *)SetImageNameForUn_Selected:(NSString *)strName{
    // changed by kalpesh as per android 30/5/2019
    return @"cluster_both_green";
}
@end
