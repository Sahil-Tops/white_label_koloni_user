...
//    UIImage *image = _backgroundImages[bucketIndex];
    return [self iconForText:text withBaseImage:[UIImage imageNamed:@"cluster"]];
  }
...
- (UIImage *)iconForText:(NSString *)text withBucketIndex:(NSUInteger)bucketIndex {
  UIImage *icon = [_iconCache objectForKey:text];
  if (icon != nil) {
      return icon;//[UIImage imageNamed:@"cluster"];
  }
  ...
  // larger buckets).
  CGFloat rectDimension = MAX(20, MAX(textSize.width, textSize.height)) + 3 * bucketIndex + 5;
  CGRect rect = CGRectMake(0.f, 0.f, rectDimension + 15, rectDimension + 15);
